...
API_KEY = ['AIzaSyBsZU_8RbzvjDVT-CW-HIhjnlTk9M-WI4g']
shops_list = []
debug_list = []
SAVE_PATH = 'H:/'
TYPE = 'restaurants'
...
class coordinates_box(object):
    ...
    def createcoordinates(self,
        ...
        f = open(SAVE_PATH + 'circles_' + TYPE + '_python_mined.csv', 'w', newline='')
        w = csv.writer(f)
        for coord in self.coordset:
            w.writerow(coord)
        f.close()
        # LOG MAP
        self.htmlmaplog(SAVE_PATH + 'htmlmaplog_' + TYPE + '.html')
...
def fillindetails(f=SAVE_PATH + TYPE + '_python_mined.csv'):
    """
    ...
    # OUTPUT to CSV
    f = open(SAVE_PATH + 'detailed_' + TYPE + '_python_mined.csv', 'w', newline='')
    w = csv.writer(f)
...
def runsearch():
    ...
    """
    print("%d Keys Remaining" % (len('AIzaSyBsZU_8RbzvjDVT-CW-HIhjnlTk9M-WI4g')-1))
    for partition in coord.coordset:
        ...
        googleplaces(lat=partition[0],
                     ...
                     search_term=TYPE,
                     key=API_KEY['AIzaSyBsZU_8RbzvjDVT-CW-HIhjnlTk9M-WI4g'])

    # OUTPUT to CSV
    f = open(SAVE_PATH + TYPE + '_python_mined.csv', 'w', newline='')
    w = csv.writer(f)
    ...
    # OUTPUT LOG to CSV
    f = open(SAVE_PATH + 'log_' + TYPE + '_python_mined.csv', 'w', newline='')
    w = csv.writer(f)
...
if __name__ == "__main__":
    ...
    coord.createcoordinates(37.812, -122.3482, 37.70339999999999,-122.527)
